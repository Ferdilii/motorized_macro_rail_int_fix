...
module motor_bracket(rail_body_width, rail_body_height) {
  motor_gap = 41.5;
  rail_ypad = 7;
  rail_zpad = 16;
  rail_overlap = 30;
  ...
  module rail_cutout() {
    ...
    cutout_z = rail_body_height + cutout_pad * 2;
    translate([
        -cutout_pad,
        -cutout_y / 2,
        -cutout_z / 2]) cube([50, cutout_y, cutout_z]);
  }
  ...
  color("green") difference() {
    ...
    flange_cutout();
    motor_holes();
    tz(bracket_z / 2 - tighten_hole_offset) tighten_holes();
    tz(-bracket_z / 2 + tighten_hole_offset) tighten_holes();
  }
}
