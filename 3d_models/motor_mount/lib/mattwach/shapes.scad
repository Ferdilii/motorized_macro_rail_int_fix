...
module rounded_cube_x(size, fillet) {
  xsize = size[0];
  ysize = size[1];
  zsize = size[2];
  translate([0, 0, zsize]) rotate([0, 90, 0]) rounded_cube([zsize, ysize, xsize], fillet);
}

module rounded_cube_y(size, fillet) {
  xsize = size[0];
  ysize = size[1];
  zsize = size[2];
  translate([
      0,
      ysize,
      zsize]) rotate([90, 90, 0]) rounded_cube([zsize, xsize, ysize], fillet);
}
