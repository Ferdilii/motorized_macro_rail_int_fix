...
module oled_128x128_1_5in_spi(include_pin_headers=true) {
  overlap = 0.01;
  ...
  pcb_thickness= 1.2;
  pcb_width = 47;
  pin_hole_top_offset = 1.4;
  pin_hole_spacing = 2.54;
  pin_count = 7;
  pin_span = pin_hole_spacing * (pin_count - 1);
  pin_xoffset = (pcb_length - pin_span) / 2;

  module pcb() {

    mount_hole_xspan = 28.8;
    mount_hole_yspan = 41.9;

    module mount_hole() {
      mount_hole_diameter = 2.85;
      tz(-overlap - bolt_ring_extrude) cylinder(
          d=mount_hole_diameter, h=pcb_thickness + bolt_ring_extrude * 2 + overlap * 2);
    }
    ...
    difference() {
      ...
      txy((pcb_length - mount_hole_xspan) / 2,
          (pcb_width - mount_hole_yspan) / 2 + mount_hole_yspan) mount_hole();
      for (i = [0:pin_count-1]) {
        tx(pin_xoffset + pin_hole_spacing * i) pin_hole();
      }
    }
  }

  module screen() {
    screen_top_offset = 4.9;
    oled_height = 37.4;
    oled_thickness = 0.7;
  ...
  module pin_headers() {
    translate([
        ...
        pcb_width - pin_hole_top_offset,
        0
    ]) ry(180) pin_header(2p54header, pin_count, 1);
  }
